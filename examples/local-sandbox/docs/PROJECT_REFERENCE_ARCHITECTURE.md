...
- Accessibility: skip-to-content link, `main` landmark (`tabIndex=-1`), `aria-label`s,
  focus-visible rings, semantic landmarks; all animation disabled under
  `prefers-reduced-motion`.
- Scroll behaviour is owned by `ScrollToTop` in `src/components/layout.tsx`: a route
  change scrolls to the top, while a hash deep link (`/#skills`) scrolls the named
  section into view and never falls back to the top before it has looked for the
  target. `html { scroll-padding-top: 5rem }` in `src/styles.css` keeps the landing
  position clear of the sticky header.
...
- Anchor tests (`src/pages/anchor-navigation.test.tsx`): a hash deep link
  (`/#skills`) scrolls the named section into view instead of jumping to the top, a
  route without a hash scrolls to the top, and a hash with no target falls back to
  the top. Regression guard for the "hash navigation broken on first load" trap.
