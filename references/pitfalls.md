...
`Origin:` names the source. **`field log`** = the reference project's working log (a live production
site on this stack; it lives outside this repository and was read read-only). **`old kit`**
...
- **Origin.** field log 2026-09-02 13:08; persisted as the true cause after A2–A4
  failed.
...
- **Check.** No local check caught it: the fix passed local Playwright 7/7 and shipped, and the
  ...
  320/375/390.
- **Origin.** field log 2026-09-02 12:10 → 12:20 REGRESSION.
...
- **Origin.** field log 2026-09-02 12:35.
...
- **Origin.** field log 2026-09-02 12:48.
...
- **Origin.** field log 2026-09-02 12:58.
...
- **Check.** Cold-cache first-load test: open `/#<anchor>` in a fresh browser context (empty cache,
  ...
  It is a **named gate** in `references/build.md` §3.1, not a local experiment.
- **Origin.** field log 2026-09-02 12:52 (registry + driver); old kit
  `SKILL_INTERACTIVE_PORTFOLIO.md` Phase 4 (deferred-section anchor paragraph, backported in
  backported later); `AGENTS.md` §3, Stage 3 step 3.
...
- **Check.** Grep the lockfile for every non-`registry.npmjs.org` host before the first push, and
  ...
  single foreign host.
- **Origin.** field log 2026-09-02 13:08; old kit
...
- **Origin.** field log 2026-09-02 11:20 and 11:19 (sitemap verification).
...
- **Check.** A grant audit that cross-references every table against function readers and performs
  ...
  alone.
- **The shape that finally settled it (2026-09-25).** "Which anon grant is dead?" is the wrong
  question, because the answer keeps changing as functions change. The settled model is: `anon`
  holds **no base-table grant at all** — every public read goes through a `*_public` view, and a
  function that needs a column the public view prunes (`cv_settings.creation_prompt`) uses the
  **service-role** client. That removes the whole class: there is no "dead grant" to misjudge, and
  a column added to a registry later is not public by default. When you touch an anon grant, keep
  the invariant and re-run the probe; never re-widen the base table to fix a function.
- **Origin.** old kit `GUIDE_FROM_SCRATCH.md` Step 14 and the security checklist ("Before revoking
  any 'dead' anon grant, verify every reader"); the 2026-09-25 hardening revision (views-only anon
  surface, `DATABASE_SCHEMA.md` §1/§3/§10).
...
- **Check.** A live audit asserting every page's meta description is ≤160 characters and the title
  ...
  live after deploy.
- **Origin.** field log 2026-09-02 11:44 and 13:21.

---

## Source map

`field log` entries come from the reference project's working log, which is **not part of this
repository** and was read read-only. `old kit` entries come from the files in this repository.

| Entry | Trap | Origin | Where recorded |
|---|---|---|---|
| A1 | iOS auto-zoom below 16px | field log | 2026-09-02 13:08 |
| A2 | Global `interactive-widget=resizes-content` regression | field log | 2026-09-02 12:10 → 12:20 |
| A3 | `overflow-x: clip` / WebKit bug 150715 | field log | 2026-09-02 12:35 |
| A4 | `top: 0` + `visualViewport.height` drawer | field log | 2026-09-02 12:48 |
| A5 | Horizontal overflow re-triggers zoom | field log | 2026-09-02 12:58 |
| B1 | Deferred-section anchors | both | field log 2026-09-02 12:52 + 12:56; old kit `SKILL_INTERACTIVE_PORTFOLIO.md` Phase 4; `AGENTS.md` Stage 3 |
| C1 | Lockfile registry re-homing | both | field log 2026-09-02 13:08; old kit `SKILL_INTERACTIVE_PORTFOLIO.md` Phase 1; `AGENTS.md` Stage 3 |
...
| C4 | Sitemap single source / 307 canonicals | field log | 2026-09-02 11:20 and 11:19 |
...
| D2 | Revoking a "dead" anon grant (`cv_settings`) | old kit + 2026-09-25 hardening revision | `GUIDE_FROM_SCRATCH.md` Step 14 + security checklist; views-only anon surface |
| D3 | Rate-limit IP header trust | old kit + 2026-09-25 hardening revision | `GUIDE_FROM_SCRATCH.md` Step 11 + Step 14; single trusted header, fail closed |
