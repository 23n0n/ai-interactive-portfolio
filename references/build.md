...
rules are in `AGENTS.md` §8 and `PLAN_AI_DISTRIBUTION.md` §6.
...
(`PLAN_AI_DISTRIBUTION.md` D4, `AGENTS.md` §6–§7) — no silent swaps; a change that genuinely cannot
...
4. Write `.npmrc` with `registry=https://registry.npmjs.org/` (§1.1).
...
All four green, and `bun run dev` serves the app. Do not start Stage 3.2 until this passes.
...
4. Create the admin account on the owner's email domain and set its `app_metadata` to
   ...
   **Authentication → Hooks → Customize Signup** to `hook_restrict_signup_by_email_domain`; the
   `check_email_domain` BEFORE INSERT trigger enforces the domain even without the hook.
...
4. Build the admin surface:
   ...
   - KB image library (public bucket, admin-only writes, 5 MB limit, MIME allow-list
     `png/jpeg/webp/gif` — **no SVG**, UUID filenames).
...
5. **Sanitize server-side.** A server-side HTML sanitizer runs on all rich content, with an
   allow-list (`<img>` over http(s) only, no `data:` URIs).
...
| `translate-recommendation` | Writes `recommendations.recommendation_text_en`; the public view serves `COALESCE(en, original)` plus `is_translated` | JWT verify, admin |
...
| `abuse-alert` | Scheduled watchdog, every 15 min via `supabase/config.toml`; thresholds from `ABUSE_*` secrets; writes `abuse_alerts`; optional counts-only webhook — never question text or PII | scheduled |
...
non-admin token → `403`; admin token → works.
...
| Sub-stage | Gate |
|---|---|
| Scaffold | `bun install --frozen-lockfile && bun run typecheck && bun run lint && bun run build` green; `bun run dev` serves the app; lockfile resolves from the public npm registry in CI |
...
| Sections | typecheck, lint, build green; browser check on desktop and mobile; every anchor reaches its deferred section on first load |
...
| Interactive features | edge-function guards reject bad method (`405` + `Allow`) and bad body (`415`); Turnstile gate rejects missing (`403`) and dummy (`invalid-input-response`) tokens; rate limits enforced; admin functions reject missing (`401`), forged (`401`) and non-admin (`403`) tokens; no secret shapes in `dist/` |
...
|---|---|
| §1 Scaffold | `SKILL_INTERACTIVE_PORTFOLIO.md` Phase 1 (Scaffold), including the dependency-source hygiene / public npm registry rule; `GUIDE_FROM_SCRATCH.md` Step 7; stack from `PLAN_AI_DISTRIBUTION.md` §9 D4 |
| load order and progressive loading | `PLAN_AI_DISTRIBUTION.md` §6; `AGENTS.md` §8 (sub-stage loading rules) |
