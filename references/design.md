...
6. The two scope decisions carried from intake: the focus/comprehensive answer (section budget) and
   the language mode (single, primary + metadata, or full UI) — both recorded in the contract.
7. **Owner gate #1:** the owner's explicit approval of tokens + sketch, recorded in
...
- A hard constraint from Q18 covers copy as well as layout: a hated phrase, buzzword or writing
  style is an exclusion for the voice, and no section may reintroduce it.
...
- **Language mode** (Q23 follow-up): single, primary + secondary metadata, or full UI. It changes
  what the layout must hold (a language switch, translated routes, or nothing visible at all) and is
  not a Stage 5 afterthought — decide it here and record it.
- **Section budget** (Q5 follow-up): a focused site gets one primary call to action and the sections
  that serve it; a comprehensive site gets the full set. Do not build sections the answer excluded.
...
- **Semantics:** one `h1` per page, landmark regions, alt text on meaningful images, labels on
  inputs, and a correct `lang` (and `hreflang`/alternate links between translated routes when the
  language mode is bilingual — see `references/distribute.md` §2).
...
- On approval, append a `design-contract` line to `decisions.log`
  ...
  approved token set lives (`styles.css`), and the two scope decisions (focus/comprehensive and
  language mode).
