...
| `recommendation_text_en` | text | English translation, drafted by the `translate-recommendation` edge function and written with `translation_reviewed = false`; the view serves it only once an admin has reviewed it |
| `translation_reviewed` | boolean NOT NULL | default `false`; set `true` by the admin ("confirm translation") — an unreviewed machine translation is never served publicly |
| `tags` | text[] | default `'{}'` |
...
| `actor_id` | uuid | `auth.uid()` of the writer; NULL for the service role |
| `actor_email` | text | resolved at write time, for a reader without a Users table |
| `operation` | text NOT NULL | `INSERT` / `UPDATE` / `DELETE` |
| `table_name` | text NOT NULL | the audited table |
| `row_id` | text | primary key of the affected row |
| `request_id` | text | from the calling function's request (never a token or a body) |
| `before_hash`, `after_hash` | text | `md5(row::text)` before and after; NULL on the side that does not exist |

Index: `admin_audit_at_idx (at desc)`. RLS: **no policy for `anon` or
`authenticated`** — the table is read by the admin panel through a
`security definer` RPC guarded by `is_admin()`; rows are written **only** by the
...
reachable ahead of the auth check. This half of the contract is a **code-review gate**: the §12
audit covers the data boundary and cannot see a function's control flow, so the ordering rule is
